...
class ChatAgent:
    ...
    async def handle_tool_calls(self, tool_calls: List[Dict[str, Any]]) -> str:
        ...
        # ------------------------------------------------------------------
        # Encapsulate tool responses in a dedicated block so the LLM can more
        # easily distinguish them from the *actual* conversation.  Wrapping
        # the payload in `<tool_output>` tags has proven to reduce accidental
        # mixing of historical snippets with the current dialogue during
        # subsequent reasoning steps.
        # ------------------------------------------------------------------

        formatted_lines: list[str] = [
            "🔧 SYSTEM NOTE: BEGIN_TOOL_OUTPUT (generated by tools – NOT user input)\n"
        ]

        import json as _json  # local alias to avoid shadowing outer scope

        for idx, r in enumerate(results, 1):
            # Wrap each individual tool result so that the model can, if
            # necessary, parse them separately.
            tool_block = (
                f"<tool_output index=\"{idx}\" name=\"{r['tool']}\">\n"
                f"Arguments: `{_json.dumps(r['arguments'], ensure_ascii=False)}`\n\n"
                f"{r['result']}\n"
                "</tool_output>\n"
            )
